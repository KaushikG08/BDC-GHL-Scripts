...
(async function() {
        ...
        window.fetch = async function(...args) {
            ...
            if (url.includes('leadconnectorhq.com') && opts?.headers) {
                ...
                if (opts.headers.Authorization && opts.headers.Authorization.startsWith("Bearer ")) {
                    capturedAuthToken = opts.headers.Authorization.split(" ")[1];
                }
                
                // Capture Token-Id
                if (opts.headers['Token-Id'] || opts.headers['token-id']) {
                    capturedTokenId = opts.headers['Token-Id'] || opts.headers['token-id'];
                }
            }
    ...
    async function openModal() {
        ...
        try {
            ...
            if (!capturedAuthToken || !capturedTokenId) {
                content.innerHTML = '<p class="text-center text-yellow-600">Waiting for authentication tokens...</p>';
            ...
            content.innerHTML = '<p class="text-center">Loading conversation data...</p>';
            ...
            if (!convRes.ok) {
                throw new Error(`Conversation fetch failed: ${convRes.status}`);
            }
            
            const contactId = (await convRes.json()).contactId;
            
            // Fetch contact data
            content.innerHTML = '<p class="text-center">Loading contact details...</p>';
            ...
            if (!contRes.ok) {
                throw new Error(`Contact fetch failed: ${convRes.status}`);
            }
            ...
            content.innerHTML = `
                <div class="text-center p-4">
                    ...
                    <p class="text-sm mb-4">Please try interacting with the app to generate authentication tokens.</p>
                    <button id="token-retry-btn" class="bg-blue-500 text-white px-4 py-2 rounded mt-2">
                        Try Again
                    </button>
                </div>`;
            document.getElementById("token-retry-btn").addEventListener("click", openModal);
    ...
    function initTokenMonitor() {
        setInterval(() => {
            if (capturedAuthToken && capturedTokenId) {
                console.log("Tokens captured and ready");
            } else {
